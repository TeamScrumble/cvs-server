...
class ProductService(
    ...
    suspend fun save(
        ...
        val products = result.data.map { it.toEntity(result.target, crawlRunId, false) }

        val savedIds = mutableListOf<Long>()

        products.chunked(chunkSize).forEach { chunk ->
            savedIds += productRepository.upsertAll(chunk, crawlRunId)
        }

        return savedIds.distinct()
    }

    suspend fun softDeleteNotInRun(target: CvsTarget, crawlRunId: String): List<Long> {
        val idsToDelete = productRepository.findIdsToSoftDelete(target, crawlRunId).toList()
        if (idsToDelete.isEmpty()) return emptyList()

        idsToDelete.chunked(1000).forEach { chunk ->
            productRepository.softDeleteByIds(chunk)
        }
